...
public class CmsStatApiAct {
	...
	public void config(Integer siteId,String siteName,String userName,String password,
		...
		if (!errors.hasErrors()) {
			...
			tjMap.put(cn.onlov.on_cms_common.core.Constants.TONGJI_TOKEN, token);
			String tongjiSiteid = AES128Util.decrypt(tjSiteId, account.getAesKey(), account.getIvKey());
			tjMap.put(cn.onlov.on_cms_common.core.Constants.TONGJI_SITEID, tongjiSiteid);
			tjMap.put(cn.onlov.on_cms_common.core.Constants.USERNAME, userName);
			tjMap.put(cn.onlov.on_cms_common.core.Constants.PASSWORD, password);
			tjMap.put(cn.onlov.on_cms_common.core.Constants.SITENAME, siteName);
