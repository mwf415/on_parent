...
import cn.onlov.on_cms_common.cms.manager.main.*;
...
import cn.onlov.on_cms_common.cms.staticpage.FtpDeleteThread;
import cn.onlov.on_cms_common.cms.entity.main.Content.ContentStatus;

import cn.onlov.on_cms_common.cms.staticpage.exception.*;
	...
	public void check(String ids,HttpServletRequest request,HttpServletResponse response){
		...
		if (!errors.hasErrors()) {
			...
			if (errors.hasErrors()) {
				...
				for (Content bean : beans) {
					if(bean.getCheckResult() != Content.CheckResultStatus.nopass){
				...
				if (channel!=null) {
					for(Integer contentId:idArr){
						...
						if(bean!=null&&channel!=null){
							...
							manager.update(CmsUtils.getUser(request), bean, ContentRecord.ContentOperateType.move);
						}
					}
					result = true;
					message = Constants.API_MESSAGE_SUCCESS;
...
//					if(ContentMngImpl.authCheck(request, errors, c.getStatus(), ContentOperateType.pigeonhole)){}
		...
		if (!errors.hasErrors()) {
			...
			if (errors.hasErrors()) {
				...
				for(int i=0;i<idArr.length;i++){
					...
					manager.update(CmsUtils.getUser(request), c, ContentRecord.ContentOperateType.reuse);
					afterContentStatusChange(c,list,ContentStatusChangeThread.OPERATE_UPDATE);
				}
				log.info("update CmsFriendlink priority.");
				result = true;
	...
	public void sendToTopic(String ids,String topicIds,HttpServletRequest request,HttpServletResponse response){
		...
		int failContent = 0;
		int successContent = 0;
		String failDetail = "[";
		String successDetail ="[";
		if (!errors.hasErrors()) {
			Integer[] idArr = StrUtils.getInts(ids);
			Integer[] topicArr = StrUtils.getInts(topicIds);
			//校验是否有不存在数据的内容ID或专题ID
			errors = validateSendToTopic(errors,request,idArr,topicArr);
			if (errors.hasErrors()) {
				message = errors.getErrors().get(0);
				code = ResponseCode.API_CODE_PARAM_ERROR;
			}else{
				List<CmsTopic> topics = topicMng.getListByIds(topicArr);
				
				for(Integer contentId:idArr){
					//校验推专题与栏目之间是否可以推送
					Content content = manager.findById(contentId);
					if(topics!=null && topics.size()>0) {
						for(CmsTopic topic:topics) {
							//判断专题是否关联栏目，如关联栏目则需要判断关联栏目是否与内容所属栏目一致，否则直接推送
							if(topic.getChannels()!=null && topic.getChannels().size()>0) {
								for (Channel channel:topic.getChannels()) {
									//如改内容所属栏目ID与专题关联栏目ID一致则推送至专题
									if(channel.getId() == content.getChannel().getId() || channel.getId().equals(content.getChannel().getId())) {
		...
		//body = "{\"result\":"+result+",\"fail\":{\"count\":"+failContent+",\"detail\":"+failDetail+"},\"success\":{\"count\":"+successContent+",\"detail\":"+successDetail+"}}";
		body = "{\"result\":"+result+"}";
		ApiResponse apiResponse = new ApiResponse(request, body, message, code);
		ResponseUtils.renderApiJson(response, request, apiResponse);
	}
